...
def fetch_address_info(public_addresses, max_retries=3, delay=20):
	...
	for attempt in range(max_retries):
		try:
			...
			print(f"\033[91mError: Failed to fetch info for addresses (attempt {attempt + 1}/{max_retries}): {error}\033[0m")
...
def process_found_keys(private_key, public_addresses, input_file, output_file):
	...
	if not address_info:
		print(f"\033[91mError: Exiting...\033[0m")
	...
	for address, data in address_info.items():
		if data['final_balance'] > 0:
			print(f"\033[92mFound: Balance at Address: {address}, Private Key: {private_key}, Balance: {data['final_balance']}\033[0m")
			with open(output_file, 'a') as file:
				file.write(f"{private_key}\n")
			return
		elif data['n_tx'] > 0:
			print(f"\033[92mFound: Transactions at Address: {address}, Private Key: {private_key}, Transactions: {data['n_tx']}\033[0m")
			with open(output_file, 'a') as file:
				file.write(f"{private_key}\n")
			return
...
if __name__ == "__main__":
	try:
		...
		print(f"\033[91mKeyboardInterrupt: Exiting...\033[0m")
		save_unprocessed_private_keys(input_file)
		dedupe_and_sort_output(output_file)
		sys.exit(1)
